package com.android.app.core.db;
...
public abstract class DataBase {
    ...
    private ArrayList<Class<? extends BaseORMModel>> klasss = getTables();

    protected abstract ArrayList<Class<? extends BaseORMModel>> getTables();
    ...
    protected <T extends BaseORMModel> ArrayList<T> query(Class<T> klass) {
    ...
    public  <T extends BaseORMModel> int del(Class<T> klass, String selectSql){
    ...
    public <T extends BaseORMModel> int update(Class<T> klass, ContentValues values, String selectSql){
        int  isDelSuccessCount = 0;
        String tabName = "";
        if (klass.isAnnotationPresent(Table.class)) {
            tabName = klass.getAnnotation(Table.class).name();
